export default async (request, context) => {
  ...
  const seed=central&&central.initialized!==false?`<script>try{localStorage.setItem('theIceBonusConfigV3',${JSON.stringify(JSON.stringify(central.bonus))});localStorage.setItem('theIceProductsV1',${JSON.stringify(JSON.stringify(central.products))})}catch(e){}</script>\n`:'';
  const compatSeed=`<script>try{const k='theIceBonusConfigV3';const c=JSON.parse(localStorage.getItem(k)||'null');if(c&&Number(c.target)===72&&Math.abs(Number(c.net)-.72)<.0001){c.target=70;c.net=.76;localStorage.setItem(k,JSON.stringify(c))}}catch(e){}</script>\n`;
  const rosterSeed=`<script>try{const k='shiftRostersV2';const base={am:['Kevin','Wilfredo','Jose','John','Marlon'],extension:[],pm:['Santiago','Amaiker','Samuel']};let r=JSON.parse(localStorage.getItem(k)||'null');if(!r||typeof r!=='object')r=base;else{if(!Array.isArray(r.am))r.am=base.am;if(!Array.isArray(r.extension))r.extension=[];if(!Array.isArray(r.pm))r.pm=base.pm;if(!r.pm.some(n=>String(n).trim().toLowerCase()==='samuel'))r.pm.push('Samuel')}localStorage.setItem(k,JSON.stringify(r))}catch(e){}</script>\n`;
  html = html.replace('<script>\n(function(){',seed+compatSeed+rosterSeed+'<script src="/config-gate.js?v=20260908c"></script>\n<script src="/bonus-config.js?v=20260908c"></script>\n<script src="/product-config.js?v=20260908c"></script>\n<script src="/central-config.js?v=20260908c"></script>\n<script src="/bonus-opening.js?v=20260908c"></script>\n<script src="/inventory-flow-v2.js?v=20260908c"></script>\n<script src="/active-tools.js?v=20260908c"></script>\n<script>\n(function(){');
  html = html.replace('const COST=37000, TARGET=70, KGBAG=15, NET=.76, REF=6, MAX_BELOW=3;\n  const TH=[190,220,260,300,360], SHARES=[1,.8,.7,.6];',`const BC=window.BONUS_CFG||{};\n  const COST=Number(BC.cost||37000), TARGET=Number(BC.target||70), KGBAG=Number(BC.eqKg||15), NET=Number(BC.net||.76), REF=6, MAX_BELOW=3;\n  const TH=(BC.thresholdKg&&BC.thresholdKg[6]?BC.thresholdKg[6].map(x=>Number(x)/KGBAG):[190,209,260,300,360]), SHARES=(Array.isArray(BC.shares)&&BC.shares.length===4?BC.shares:[.55,.50,.40,.30]);`);
  ...
  html = html.replace('const labor=p*COST, sav=Math.max(0,TARGET*kg-labor);','const scale=BC.scale&&BC.scale[p]?Number(BC.scale[p]):1, labor=p*COST, sav=Math.max(0,(TARGET*scale)*kg-labor);');
